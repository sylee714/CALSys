...
def get_lowest_similarity(positive_case_sample, negative_case_list):
    ...
    for sample in negative_case_list:
        negative_case_text_list.append(nlp(sample[5]))
...
def find_negative_case(positive_case_sample, negative_case_list):
    ...
    min_date = positive_assign_date - pd.DateOffset(days=5)
    max_date = positive_assign_date + pd.DateOffset(days=5)
...
data = pd.read_csv("../Files/zero_day_exploits_with_description.csv")
...
for index, entry in data.iterrows():
    if entry['Zero Day Exploit']:
        positive_bool_list.append(True)
        negative_bool_list.append(False)
    else:
        positive_bool_list.append(False)
        negative_bool_list.append(True)

positive_cases = data.loc[positive_bool_list]
negative_cases = data.loc[negative_bool_list]
...
selected_negative_cases_df = pd.DataFrame(selected_negative_cases, columns=['CVE-ID', 'Patch Date', 'Exploit Date',
                                                                            'Zero Day Exploit', 'MITRE Assign Date',
                                                                            'CVE Description', 'Label'])
selected_negative_cases_df.to_csv("selected_zero_day_negative_cases_with_zdi.csv", index=False)
negative_cases.to_csv("remaining_zero_day_negative_cases_with_zdi.csv", index=False)
positive_cases.to_csv("selected_zero_day_positive_cases_with_zdi.csv", index=False)
